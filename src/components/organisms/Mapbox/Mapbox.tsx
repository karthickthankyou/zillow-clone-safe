...
const MapBox = ({
  ...
  const [viewport, setViewport] = useState(() => initialLocation)
  const [dragging, setDragging] = useState(false)

  useEffect(() => {
    if (!dragging) setLocation(viewport)
  }, [dragging, setLocation, viewport])
  ...
  return (
    <div className={`relative w-full ${className} `}>
      <ReactMapGL
        ...
        onInteractionStateChange={({ isDragging }) => setDragging(isDragging)}
      >
        {/* <GeolocateControl /> */}
